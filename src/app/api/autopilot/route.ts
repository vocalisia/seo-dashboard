...
/**
 * True if the query looks like a different written language than the article target
 * (e.g. DE/IT keywords when generating FR) — avoids mixed-language junk from GSC.
 */
function keywordConflictsArticleLanguage(raw: string, articleLang: string): boolean {
  ...
  for (const o of otherLangs) {
    // English tokens are common in mixed FR/DE/IT commercial queries — do not reject on "the/best/how".
    ...
    // 2+ indicator tokens from another language → likely wrong language
...
export function isUnusableSeoKeyword(
  ...
  // Google power-user / operator queries (often slip into GSC)
  ...
  // URL-like or domain-like (navigational, not content keywords)
  ...
  // Single-token queries: often navigational (brand) or too vague
  if (tokens.length === 1) {
    const t = alphaOnly(tokens[0]);
    if (t.length > 0 && t.length < 6) return true;
    if (brandHints.has(t)) return true;
    // Very long single tokens are usually OK (e.g. "cryptocurrency"); short ones are risky
  ...
  // Multi-token: reject if entire query is only brand tokens (e.g. "trust vault" on Trust Vault)
  if (tokens.length >= 2) {
...
function pickFirstUsableKeyword(
  rows: KeywordRow[],
  site: { url: string; name: string },
  articleLang: string
): KeywordRow[] {
